...
![](./res/python-idle.png)
### 安装pip
如果你在CentOS上使用 pip 命令时出现 "未找到命令" 的错误，这可能是因为 pip 不在系统的PATH环境变量中，或者没有安装 pip。

在较新的CentOS版本中，通常使用 pip 之前需要手动安装 python3-pip 包。你可以使用以下命令来安装 python3-pip：

sudo yum install python3-pip
如果你的系统上使用的是Python 2，可以使用以下命令来安装 python2-pip：

sudo yum install python2-pip
一旦安装了pip，你应该能够使用 pip install ipython 来安装IPython。如果你使用Python 3，则使用 pip3 install ipython。

#### pip问题
使用阿里源安装ipython ：
pip3 install -i https://mirrors.aliyun.com/pypi/simple/ ipython3

我安装了sudo yum install python3-pip
但现在pip3 install ipython
-bash: /opt/Python37/bin/pip3: /opt/Python-3.7.6/bin/python3.7: 坏的解释器: 没有那个文件或目录

错误消息表明你的pip3 尝试使用了错误的Python解释器路径 /opt/Python-3.7.6/bin/python3.7。这可能是因为你的系统中有多个Python版本，并且pip3 尝试使用不正确的版本。为了解决这个问题，你可以采取以下步骤：

首先，确定你的系统上已经安装了Python 3，并且你可以通过运行 python3 命令来启动它。如果没有安装Python 3，你需要安装它。

使用以下命令查找正确的Python 3解释器路径：

which python3
这将返回正确的Python 3解释器的路径，例如 /usr/bin/python3。

一旦你知道了正确的Python 3解释器路径，你可以使用该路径来安装IPython。例如，如果正确的路径是 /usr/bin/python3，你可以运行以下命令来安装IPython：

/usr/bin/python3 -m pip3 install ipython
这将使用正确的Python 3解释器来安装IPython。确保将 /usr/bin/python3 替换为你在步骤2中找到的路径。

这应该能够解决你遇到的问题。如果你仍然遇到困难或有其他问题，请提供更多信息，以便我可以提供更具体的帮助。
#### IPython - 更好的交互式编程工具
